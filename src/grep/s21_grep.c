...
void parsing(int argc, char *argv[]) {
  ...
  char buffer[BUFFER];
  char pattern[BUFFER];
  char file_name_buffer[225];
  size_t buffer_size = BUFFER * sizeof(char);
  if ((buffer_size >= sizeof(pattern)) && (buffer_size >= sizeof(buffer)) &&
      (buffer_size >= sizeof(file_name_buffer))) {
...
void other_pattern_cr(int argc, char *argv[], char *pattern,
  ...
  if (flags->f) {
    FILE *f = fopen(file_name_buffer, "r");
    if (f != NULL) {
      ...
      while (fgets(buffer, BUFFER, f)) {
        int lb = strlen(buffer);
        if (buffer[lb - 1] == 10 || buffer[lb - 1] == 13) {
          buffer[lb - 1] = '|';
        }
        if (pattern[0] != 1) {
          strncat(pattern, buffer, lb);
...
void grep_reader(int argc, char *argv[], char *pattern, grep_options *flags) {
  ...
  while (current_file < argc) {
    f = fopen(argv[current_file], "r");
    if (f != NULL) {
      grep(argc, argv, flags, f, pattern, current_file);
      fclose(f);
    } else {
      if (!flags->s) {
